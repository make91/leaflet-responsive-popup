...
L.ResponsivePopup = L.Popup.extend({
	...
	options: {
		hasTip: true,
		/** Array of preferred position of popup, can include any of 'top', 'left', 'bottom', 'right' */
		positionPreference: []
		/*
	...
	_initLayout: function () {
		...
		if (!this.options.positionPreference) {
			this.options.positionPreference = [];
		}
		const defaultPositionPreference = ['top', 'left', 'bottom', 'right'];
		for (let position of defaultPositionPreference) {
			if (!this.options.positionPreference.includes(position)) {
				this.options.positionPreference.push(position);
			}
		}
		if (this.options.positionPreference.length > 4) {
			console.error("Invalid position preference, using default ['top', 'left', 'bottom', 'right']");
			this.options.positionPreference = defaultPositionPreference;
		}
	},
	...
	_updatePosition: function () {
		...
		// position the popup (default order of preference is: top, left, bottom, right, centerOnMap)
		let popupOk = false;
		for (const direction of this.options.positionPreference) {

			if (direction === 'top' && canGoTop) {
			...
			} else if (direction === 'bottom' && canGoBottom) {
